...
func CreateCustomCommand(name, usage, description, rconCommand, permissions, requirementType string, minArgs, maxArgs, minPower int, isBuiltIn bool) error {
	db := database.DB
...
func GetCustomCommand(name string) (*CustomCommand, error) {
	...
	err := db.Where("name = ? AND enabled = ?", name, true).First(&cmd).Error
	if err != nil {
		return nil, err
	}
	return &cmd, nil
...
func GetAllCustomCommands() ([]CustomCommand, error) {
	...
	err := db.Order("name").Find(&commands).Error
	return commands, err
}
